...
class LocalResourceProvider:
            ...
            record = (
                session.execute(
                    select(ResourceRecord).where(
                        ...
                        ResourceRecord.provider == self.name,
                    )
                )
                .scalars()
                .first()
    ...
    def _list_records(
        ...
        *,
        ...
        stmt = select(ResourceRecord).where(ResourceRecord.provider == self.name)
        if owner_id:
    ...
    def _load_legacy_records(self) -> None:
        ...
        with SessionLocal() as session:
            existing_ids = set(
                session.execute(select(ResourceRecord.resource_id)).scalars().all()
            )
            for stored in legacy_items:
                if stored.resource_id in existing_ids:
                    continue
                session.add(_stored_to_record(stored))
            session.commit()
...
class DbResourceProvider:
    ...
    def delete(self, resource_id: str) -> None:
        ...
        if cache_path.exists():
            cache_path.unlink()
        with SessionLocal() as session:
            payload = session.get(ResourcePayloadRecord, resource_id)
            record = session.get(ResourceRecord, resource_id)
