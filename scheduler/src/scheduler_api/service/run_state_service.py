...
from scheduler_api.infra.catalog import PackageCatalogError, catalog
from scheduler_api.service.facade import resource_services
from scheduler_api.domain.models import DispatchRequest, FrameRuntimeState, NodeState, RunRecord, FINAL_STATUSES, _utc_now
from scheduler_api.domain.bindings import _merge_result_updates
from scheduler_api.domain.graph import apply_edge_bindings, apply_frame_edge_bindings, apply_middleware_output_bindings
from scheduler_api.domain.frames import activate_frame, build_container_frames, current_frame, pop_frame
from scheduler_api.engine.events.format import build_workflow_snapshot
from scheduler_api.engine import dispatch, frames, initialise, lifecycle, lookup, status
from scheduler_api.engine.events import emit
from scheduler_api.engine.next import handle_next_request as process_next_request
from scheduler_api.engine.pending import (
...
from scheduler_api.engine.updates import (
    apply_command_error,
    apply_feedback,
    apply_record_result,
)
...
def _load_package_vault_requirements(
    package_name: str,
    package_version: str,
    ...
    if manifest and getattr(manifest, "requirements", None):
        requirements = list(getattr(manifest.requirements, "vault", []) or [])
...
class RunStateService:
    ...
    async def create_run(
        ...
        *,
        ...
        async with self._lock:
            record = initialise.build_run_record(
                run_id=run_id,
                request=request,
                tenant=tenant,
                owner_id=owner_id,
    ...
    async def collect_ready_nodes(self, run_id: Optional[str] = None) -> List[DispatchRequest]:
        ...
        async with self._lock:
            ...
            for record in records:
                if not record or record.status in FINAL_STATUSES:
                    continue
                owner_ids[record.run_id] = record.owner_id or record.tenant
        ...
        if state_events:
            ...
            await asyncio.gather(*tasks)
        if requests and owner_ids:
            self._apply_vault_bindings(requests, owner_ids=owner_ids)
        return requests
    ...
    async def record_result(
        ...
        if outcome.ready and record:
            self._apply_vault_bindings(outcome.ready, owner_ids={record.run_id: record.owner_id or record.tenant})
    ...
    def _apply_vault_bindings(
        ...
        *,
        ...
        for request in requests:
            owner_id = owner_ids.get(request.run_id)
            if not owner_id:
                continue
            requirements = _load_package_vault_requirements(
            ...
            for requirement in requirements:
                key = str(getattr(requirement, "key", "") or "").strip()
                if not key:
                    continue
                value = resource_services.package_vault.get_value(
                    owner_id=owner_id,
                ...
                if value is None:
                    if _is_required(requirement):
                        errors.append({"key": key, "error": "missing_vault"})
                    continue
                binding: Dict[str, Any] = {
                    "source": "vault",
                    "key": key,
                    "type": getattr(requirement, "type", None),
                    "value": value,
                }
                metadata = getattr(requirement, "metadata", None)
