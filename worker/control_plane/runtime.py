...
from worker.transport.websocket import WebSocketTransport
        ...
        async def default_package_handler(action: str, message: dict) -> None:
            ...
            try:
                if action == "install":
                    ...
                    connection.package_inventory.append(
                        {"name": descriptor.name, "version": descriptor.version, "status": "installed"}
                    )
                    connection.package_manifests.append(
                        {"name": descriptor.name, "version": descriptor.version, "manifest": descriptor.manifest}
                    )
                    connection._ensure_layers()
                    assert connection.biz is not None
                    await connection.biz.send_package_event(
                        PackageEvent(
                    ...
                    connection.package_inventory = [
                        pkg
                        for pkg in connection.package_inventory
                        if not (pkg.get("name") == name and pkg.get("version") == version)
                    ]
                    connection.package_manifests = [
                        manifest for manifest in connection.package_manifests if not (
                            manifest.get("name") == name and manifest.get("version") == version
                        )
                    ]
                    connection._ensure_layers()
                    assert connection.biz is not None
...
async def start_control_plane(connection: ControlPlaneClient | None = None) -> ControlPlaneClient:
...
async def run_forever() -> None:
    """Convenience entry point that boots the control-plane and blocks until cancelled."""

    connection = await start_control_plane()
    try:
        await asyncio.Future()  # block indefinitely until cancelled
