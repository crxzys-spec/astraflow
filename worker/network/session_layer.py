...
class SessionLayer:
    ...
    def build_register_payload(self) -> RegisterPayload:
        ...
        return RegisterPayload(
            ...
            packages=self.package_inventory or [],
            manifests=self.package_manifests or None,
        )
    ...
    def build_heartbeat_payload(self) -> HeartbeatPayload:
        ...
        if self.metrics_provider:
            try:
                extra = self.metrics_provider()
            except Exception:  # noqa: BLE001
                LOGGER.debug("Suppress heartbeat metrics provider error", exc_info=True)
            else:
